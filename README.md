...
🧪 Testing Guide
Open the form (use Incognito for fresh sessions)
Scroll, focus fields, submit with a new email
Login to the dashboard → lead appears with Hot/Cold + source
Events appear in Live Events Feed within 5s
UTM test: ?utm_source=facebook&utm_medium=cpc → "Pay per Click Ads"; without medium → "Facebook"; no UTM → "Direct Traffic"
Duplicate test: resubmit an existing email → lead updates + re-scores
